...
def get_streams(client):
    ...
    for i, stream in enumerate(streams):
        ...
        print("writing new stream: number", i)
...
def main(my_client_id, my_oauth_token):
    client = twitch.TwitchHelix(client_id=my_client_id,
                         oauth_token=my_oauth_token)
    p = multiprocessing.Process(
        target=get_streams, name="Get_streams", args=(client,))
    p.start()
    time.sleep(1)  # number of streams must be max 100 for other calls to work
    if p.is_alive():
        p.terminate()
        p.join()
...
if __name__ == "__main__":
    my_client_id = "wmy914zrfeurskr6lgz2v5o0xw5fgg"
    my_oauth_token = "uvqa3v32ikm8pbygmbui2k6wzv4qjw"
